...
display = ST7789(dc_pin=22, rst_pin=18)
...
- `jetson-orin-st7789-adafruit.dts` - Device tree overlay source
- `install.sh` - Automated installation script

## What It Configures

The overlay configures:
- **Pin 18** (spi3_cs0_py3) as GPIO for RST (Reset)
...
Use the Adafruit overlay when:
- You have an Adafruit 2.0" 320x240 IPS TFT display
- Your display is already wired according to Adafruit's documentation
- You're following Adafruit tutorials for Raspberry Pi
- Your display uses pins 18 and 22 for RST and DC
...
ls -l /boot/jetson-orin-st7789-adafruit.dtbo

# Check boot config
cat /boot/extlinux/extlinux.conf | grep overlays
# Should show: overlays=/boot/jetson-orin-st7789-adafruit.dtbo
```
...
3. **Verify GPIO pins:**
   ```bash
   sudo gpioinfo | grep -E "spi3_cs0_py3|spi3_miso_py1"
...
5. **Test with Python driver:**
   ```bash
   cd jetson-orin-st7789
   uv run st7789-test --wiring adafruit
   ```

### Wrong Python Preset

Make sure you use `wiring='adafruit'` in Python code:
...
cd ../jetson-default  # or ../waveshare
sudo ./install.sh
...
# Change wiring='adafruit' to wiring='jetson' or wiring='waveshare'
```
...
# Remove: overlays=/boot/jetson-orin-st7789-adafruit.dtbo

# 3. Save and reboot
sudo reboot

# 4. Optional: Remove overlay file
sudo rm /boot/jetson-orin-st7789-adafruit.dtbo
```

## Success Checklist

- [ ] `install.sh` completed successfully
- [ ] Overlay installed to `/boot/jetson-orin-st7789-adafruit.dtbo`
...
- [ ] Python driver works with `wiring='adafruit'`
