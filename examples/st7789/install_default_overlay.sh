#!/bin/bash
# Install Default ST7789 Device Tree Overlay for Jetson Orin
...
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Default ST7789 Overlay Installer                           ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Check root
if [ "$EUID" -ne 0 ]; then 
    echo -e "${RED}✗ Please run as root: sudo ./install_default_overlay.sh${NC}"
    exit 1
fi
...
# Check if JetsonDefault entry already exists
if grep -q "LABEL JetsonDefault" "$EXTLINUX"; then
    echo -e "${YELLOW}⚠ JetsonDefault boot entry already exists${NC}"
    echo -e "${YELLOW}⚠ Removing old entry...${NC}"
    # Remove old entry
    sed -i '/^LABEL JetsonDefault$/,/^LABEL\|^$/{ /^LABEL JetsonDefault$/d; /^LABEL [^J]/Q; d; }' "$EXTLINUX"
fi
...
echo -e "${GREEN}✓ Added JetsonDefault boot entry to $EXTLINUX${NC}"

# Set as default
echo -e "${BLUE}ℹ Setting JetsonDefault as default boot option...${NC}"
sed -i 's/^DEFAULT .*/DEFAULT JetsonDefault/' "$EXTLINUX"

echo -e "${GREEN}✓ Set JetsonDefault as default${NC}"
...
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${GREEN}✓ Default overlay installed${NC}"
echo -e "${GREEN}✓ Created new boot entry: 'JetsonDefault'${NC}"
echo -e "${GREEN}✓ Set as default boot option${NC}"
echo ""
echo "  Pin 19 (MOSI) -> SPI1 MOSI"
echo "  Pin 23 (SCLK) -> SPI1 SCK"
echo "  Pin 24 (CS)   -> SPI1 CS0"
echo "  Pin 29 (DC)   -> GPIO01"
echo "  Pin 31 (RST)  -> GPIO11"
echo ""
echo -e "${YELLOW}⚠ REBOOT REQUIRED${NC}"
echo ""
echo "After reboot, verify with:"
echo "  sudo python3 tools/pin_inspector.py 29"
echo "  sudo python3 tools/pin_inspector.py 31"
echo ""
echo "Then test with:"
echo "  sudo python3 test_jetson_default.py"
echo ""
echo "To revert to original config, edit $EXTLINUX and change:"
echo "  DEFAULT JetsonDefault  ->  DEFAULT primary"
echo ""
