...
if [ ! -f "$DTS_FILE" ]; then
    echo -e "${RED}ERROR: File not found: $DTS_FILE${NC}"
    exit 1
fi

# Check if it's a .dts file
if [[ ! "$DTS_FILE" =~ \.dts$ ]]; then
    echo -e "${RED}ERROR: File must have .dts extension${NC}"
    exit 1
fi
...
if ! command -v dtc &> /dev/null; then
    echo -e "${YELLOW}WARNING: dtc not found, installing device-tree-compiler...${NC}"
    apt-get update -qq
    apt-get install -y device-tree-compiler
    echo -e "${GREEN}SUCCESS: dtc installed${NC}"
else
    echo -e "${GREEN}SUCCESS: dtc is available${NC}"
fi

# Compile
echo ""
echo -e "${BLUE}Compiling device tree overlay...${NC}"
...
echo -e "${BLUE}Output size: $SIZE bytes${NC}"
...
# Install to /boot/
echo ""
echo -e "${BLUE}Installing to /boot/$BASENAME...${NC}"
cp "$DTBO_FILE" /boot/
...
echo -e "${GREEN}SUCCESS: Installed to /boot/$BASENAME${NC}"

# Verify
if [ -f "/boot/$BASENAME" ]; then
    INSTALLED_SIZE=$(stat -c%s "/boot/$BASENAME")
    echo -e "${GREEN}SUCCESS: Verified /boot/$BASENAME ($INSTALLED_SIZE bytes)${NC}"
else
    echo -e "${RED}ERROR: Verification failed - file not found in /boot/${NC}"
    exit 1
fi
...
echo -e "${BLUE}  APPEND \${cbootargs} ... overlays=/boot/$BASENAME${NC}"
echo ""
echo "Or use the automated installer scripts in each overlay directory"
echo "which handle the boot configuration automatically."
echo ""
